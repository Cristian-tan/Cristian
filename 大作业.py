...
def HRRN():
    ...
    for m in range(len(hrrn_pcb)):
        # 优先权 = 等待时间 + 要求服务时间 / 要求服务时间
        yxq = float((hrrn_pcb[m][3] - hrrn_pcb[m][1] + hrrn_pcb[m][2]) / hrrn_pcb[m][2])
        hrrn_pcb[m].append(yxq)
        # 按照“优先权”对列表进行升序 排, x:x[7]按照优先权排序
    hrrn_pcb.sort(key=lambda x: (x[1], x[7]))
    for f in range(n):
            if (f == 0):
                startTime = int(hrrn_pcb[f][1])
                hrrn_pcb[f][3] = startTime
                hrrn_pcb[f][4] = startTime + int(hrrn_pcb[f][2])
            else:
                startTime = hrrn_pcb[f - 1][4]
                hrrn_pcb[f][3] = startTime
                hrrn_pcb[f][4] = startTime + int(hrrn_pcb[f][2])
                f += 1
        # 计算周转、带权时间
    for f in range(n):
            hrrn_pcb[f][5] = float(hrrn_pcb[f][4] - int(hrrn_pcb[f][1]))  # 周转时间
            hrrn_pcb[f][6] = float(hrrn_pcb[f][5] / int(hrrn_pcb[f][2]))  # 带权周转时间
            f += 1
            # 计算平均周转时间
            和带权SzzTime = 0
            SdqzzTime = 0
            for f in range(n):
                SzzTime = float(SzzTime + float(hrrn_pcb[f][5]))
            SdqzzTime = float(SdqzzTime + float(hrrn_pcb[f][6]))
            AzzTime = float(SzzTime / n)
            AdqzzTime = float(SdqzzTime / n)
            # 输出结果，按照开始时间进行排序
            pcb.sort(key=lambda x: x[3])
            print('运行结果 :')
            for f in range(n):
                print("时刻 : %d 开始运行进程 开始运行进程 开始运行进程 : %s 完成时间 完成时间 完成时间 : %d 周转时间 周转时间 周转时间 : %d 带权周转时 带权周转时 带权周转时 间: %.2f" \
                      % (int(hrrn_pcb[f][3]), hrrn_pcb[f][0], int(hrrn_pcb[f][4]), int(hrrn_pcb[f][5]), float(hrrn_pcb[f][6])))
            print('本次调度的平均周转时间为： %.2f' % float(AzzTime))
            print('本次调度的平均带权周转时间为： %.2f' % float(AdqzzTime))
...
if __name__ == '__main__':
    ...
    print("算法三：最高响应比优先算法HRRN")
    m = 1
    while (m == 1):
        option = int(input("请输入你要选择的算法(输入1，2，3进行选择，其他键退出):"))
        if (option == 1):
            FCFS()
        elif (option == 2):
            SJF()
